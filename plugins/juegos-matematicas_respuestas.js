...
const handler = async (m, {conn}) => {
  ...
  if (!(m.chat in global.math)) return conn.reply(m.chat, `_*< JUEGOS - MATEMÁTICAS />*_\n\n*[ ℹ️ ] El ejercicio matemático ya fue resuelto anteriormente.*`, m);
  ...
  if (m.quoted.id == global.math[id][0].id) {
    ...
    if (m.text == math.result) {
      conn.reply(m.chat, `_*< JUEGOS - MATEMÁTICAS />*_\n\n▢ *¡Respuesta correcta!*\n\n▢ *Ganaste:* ${math.bonus} XP`, m);
      ...
      if (--global.math[id][2] == 0) {
        conn.reply(m.chat, `_*< JUEGOS - MATEMÁTICAS />*_\n\n▢ *Ya no tienes más oportunidades.*\n\n▢ *Respuesta correcta:* ${math.result}`, m);
